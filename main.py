...
def drawStl(ax, nodes, elems):
    global draw_stl_list
    global draw_result
    global node_scatter
    global quiver_plots
    global fixed_node_nums
    global applied_forces
    # すでに描画されているメッシュの消去
    for draw in draw_stl_list:
        draw.remove()
    draw_stl_list.clear()
    # すでに描画されている解析結果の消去
    ...
    draw_result.clear()
    # すでに描画されているノードの消去
    for scatter in node_scatter:
        scatter.remove()
    node_scatter.clear()
    # すでに描画されている力ベクトルの消去
    for plot in quiver_plots:
        plot[1].remove()
    quiver_plots.clear()
    # 固定端リストの消去
    fixed_node_nums.clear()
    # 力点リストの消去
    ...
    for elem in elems:
        ...
        draw_stl_list.append(ax.add_collection3d(Poly3DCollection(verts, edgecolor="k", alpha=0.1)))

    # ノードのプロットと選択イベント
    for i in range(len(nodes)):
        node_scatter.append(ax.scatter(nodes[i, 0], nodes[i, 1], nodes[i, 2], color="blue", picker=True))

def read_stl_button_pressed():
    global nodes, elems, ax, canvas
    file_path = entry_read_stl.get()
    if file_path == "":
        return
    
    nodes, elems = readStl(file_path)
    drawStl(ax, nodes, elems)
...
def start_analysis():
    ...
    for i in range(len(fixed_node_nums)):
        bound.addSPC(fixed_node_nums[i] + 1, 0.0, 0.0, 0.0)
...
label_read_stl = tk.Label(control_frame, text="読み込むSTLファイルパス：")
label_read_stl.pack(anchor=tk.NW, pady=5)

entry_read_stl = ttk.Entry(control_frame)
entry_read_stl.pack(anchor=tk.NW, pady=5)

button_read_stl = tk.Button(control_frame, text="読み込み", command=read_stl_button_pressed)
button_read_stl.pack(anchor=tk.NW, pady=5)
